...
function App() {
  ...
  /* useEffect(() => {
    ...
    .transition().duration(1000).style('height', (bar : any) => `${bar.count}px`)
 ...
 .attr('d', d3.line<any>().x(dt => xScale(dt.x)) // Set the path to our line function, but where x is the corresponding x
 .y(yScale(0))).attr("stroke", "blue").attr('fill', 'none') // Set the y to always be 0 and set stroke and fill color
  ...
  .attr('d', line as any) // Set the path to our line variable (Which corresponds the actual path of the data)
